...
Current implementation status lives in the main [README](../README.md). The homepage overview tracks the local SQLite/Playwright pipeline, Tabulator management UI, backlog resolver controls, and audit model. The flowcharts document gives a visual reference for each MVP workflow. The maintenance document covers scheduled SQLite optimization. The remote migration document covers Ubuntu service-user setup and runtime data migration. The architecture document is a future-state platform design for multi-source durable collection.
